...
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleLike = async (postId) => {
    try {
      await API.put(`/posts/like/${postId}`, {}, { headers: { Authorization: token } });
      fetchPosts();
    } catch (err) {
  ...
  const toggleFollow = async (userId) => {
    try {
      await API.put(`/users/follow/${userId}`, {}, { headers: { Authorization: token } });
  ...
  const styles = {
    ...
    actionBtn: { border: "none", background: "none", color: "#65676b", fontWeight: 600, cursor: "pointer", fontSize: 14 },
    commentWrapper: { display: "flex", gap: 8, marginTop: 8 },
    commentInput: { flex: 1, border: "1px solid #e5e7eb", borderRadius: 20, padding: "8px 12px" },
    commentBtn: { padding: "8px 12px", borderRadius: 20, fontWeight: 600, backgroundColor: "#1877f2", color: "white", border: "none", cursor: "pointer" },
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <input
        ...
        style={{ width: "90%", margin: "10px auto", display: "block", padding: 8, borderRadius: 8, border: "1px solid #ccc" }}
      />

      <div style={styles.feedWrapper}>
        {filteredPosts.length === 0 && (
          <div style={{ gridColumn: "span 3", padding: 40, textAlign: "center", background: "white", borderRadius: 8 }}>
            📭 No posts found.
          </div>
        )}
            ...
            <div style={styles.postHeader}>
              <div style={styles.userInfo}>
                {post.userId?.profilePhoto ? (
                  ...
                  <div style={styles.userAvatar(getAvatarColor(post.userId?.username || "User"))}>
                    {getAvatarInitials(post.userId?.username || "User")}
                  </div>
                )}
                <strong>{post.userId?.username || "Unknown"}</strong>
              </div>
              <button style={styles.actionBtn} onClick={() => toggleFollow(post.userId?._id)}>
